 ...
 *  - If the user selected New Game, advance to Task_NewGameCarterSpeech_Init.
 ...
 *
 * Task_NewGameCarterSpeech_Init
 *  - Load the sprites for the intro speech, start playing music
 * Task_NewGameCarterSpeech_WaitToShowCarter
 *  - Spawn Task_NewGameCarterSpeech_FadeInTarget1OutTarget2
 *  - Spawn Task_NewGameCarterSpeech_FadePlatformOut
 *  - Both of these tasks destroy themselves when done.
 * Task_NewGameCarterSpeech_WaitForSpriteFadeInWelcome
 * Task_NewGameCarterSpeech_ThisIsAPokemon
 *  - When the text is done printing, spawns Task_NewGameCarterSpeechSub_InitPokeball
 * Task_NewGameCarterSpeech_MainSpeech
 * Task_NewGameCarterSpeech_AndYouAre
 * Task_NewGameCarterSpeech_StartCarterLotadPlatformFade
 * Task_NewGameCarterSpeech_StartCarterLotadPlatformFade
 * Task_NewGameCarterSpeech_SlidePlatformAway
 * Task_NewGameCarterSpeech_StartPlayerFadeIn
 * Task_NewGameCarterSpeech_WaitForPlayerFadeIn
 * Task_NewGameCarterSpeech_BoyOrGirl
 * Task_NewGameCarterSpeech_WaitToShowGenderMenu
 * Task_NewGameCarterSpeech_ChooseGender
 *  - Animates by advancing to Task_NewGameCarterSpeech_SlideOutOldGenderSprite
 *    whenever the player's selection changes.
 *  - Advances to Task_NewGameCarterSpeech_WhatsYourName when done.
 *
 * Task_NewGameCarterSpeech_SlideOutOldGenderSprite
 * Task_NewGameCarterSpeech_SlideInNewGenderSprite
 *  - Returns back to Task_NewGameCarterSpeech_ChooseGender.
 *
 * Task_NewGameCarterSpeech_WhatsYourName
 * Task_NewGameCarterSpeech_WaitForWhatsYourNameToPrint
 * Task_NewGameCarterSpeech_WaitPressBeforeNameChoice
 * Task_NewGameCarterSpeech_StartNamingScreen
 * C2_NamingScreen
 *  - Returns to CB2_NewGameCarterSpeech_ReturnFromNamingScreen when done
 * CB2_NewGameCarterSpeech_ReturnFromNamingScreen
 * Task_NewGameCarterSpeech_ReturnFromNamingScreenShowTextbox
 * Task_NewGameCarterSpeech_SoItsPlayerName
 * Task_NewGameCarterSpeech_CreateNameYesNo
 * Task_NewGameCarterSpeech_ProcessNameYesNoMenu
 *  - If confirmed, advance to Task_NewGameCarterSpeech_SlidePlatformAway2.
 *  - Otherwise, return to Task_NewGameCarterSpeech_BoyOrGirl.
 *
 * Task_NewGameCarterSpeech_SlidePlatformAway2
 * Task_NewGameCarterSpeech_ReshowCarterLotad
 * Task_NewGameCarterSpeech_WaitForSpriteFadeInAndTextPrinter
 * Task_NewGameCarterSpeech_AreYouReady
 * Task_NewGameCarterSpeech_ShrinkPlayer
 * Task_NewGameCarterSpeech_WaitForPlayerShrink
 * Task_NewGameCarterSpeech_FadePlayerToWhite
 * Task_NewGameCarterSpeech_Cleanup
 *  - Advances to CB2_NewGame.
 *
 * Task_NewGameCarterSpeechSub_InitPokeball
 *  - Advances to Task_NewGameCarterSpeechSub_WaitForLotad
 * Task_NewGameCarterSpeechSub_WaitForLotad
...
static u8 sCarterSpeechMainTaskId;
...
static void Task_NewGameCarterSpeech_Init(u8);
static void Task_DisplayMainMenuInvalidActionError(u8);
static void AddCarterSpeechObjects(u8);
static void Task_NewGameCarterSpeech_WaitToShowCarter(u8);
static void NewGameCarterSpeech_StartFadeInTarget1OutTarget2(u8, u8);
static void NewGameCarterSpeech_StartFadePlatformOut(u8, u8);
static void Task_NewGameCarterSpeech_WaitForSpriteFadeInWelcome(u8);
static void NewGameCarterSpeech_ShowDialogueWindow(u8, u8);
static void NewGameCarterSpeech_ClearWindow(u8);
static void Task_NewGameCarterSpeech_ThisIsAPokemon(u8);
static void Task_NewGameCarterSpeech_MainSpeech(u8);
static void NewGameCarterSpeech_WaitForThisIsPokemonText(struct TextPrinterTemplate *, u16);
static void Task_NewGameCarterSpeech_AndYouAre(u8);
static void Task_NewGameCarterSpeechSub_WaitForLotad(u8);
static void Task_NewGameCarterSpeech_StartCarterLotadPlatformFade(u8);
static void NewGameCarterSpeech_StartFadeOutTarget1InTarget2(u8, u8);
static void NewGameCarterSpeech_StartFadePlatformIn(u8, u8);
static void Task_NewGameCarterSpeech_SlidePlatformAway(u8);
static void Task_NewGameCarterSpeech_StartPlayerFadeIn(u8);
static void Task_NewGameCarterSpeech_WaitForPlayerFadeIn(u8);
static void Task_NewGameCarterSpeech_BoyOrGirl(u8);
static void LoadMainMenuWindowFrameTiles(u8, u16);
static void DrawMainMenuWindowBorder(const struct WindowTemplate *, u16);
static void Task_HighlightSelectedMainMenuItem(u8);
static void Task_NewGameCarterSpeech_WaitToShowGenderMenu(u8);
static void Task_NewGameCarterSpeech_ChooseGender(u8);
static void NewGameCarterSpeech_ShowGenderMenu(void);
static s8 NewGameCarterSpeech_ProcessGenderMenuInput(void);
static void NewGameCarterSpeech_ClearGenderWindow(u8, u8);
static void Task_NewGameCarterSpeech_WhatsYourName(u8);
static void Task_NewGameCarterSpeech_SlideOutOldGenderSprite(u8);
static void Task_NewGameCarterSpeech_SlideInNewGenderSprite(u8);
static void Task_NewGameCarterSpeech_WaitForWhatsYourNameToPrint(u8);
static void Task_NewGameCarterSpeech_WaitPressBeforeNameChoice(u8);
static void Task_NewGameCarterSpeech_WaitThenShowBralenMenu(u8);
static void Task_NewGameCarterSpeech_ProcessBralenMenu(u8);
static void Task_NewGameCarterSpeech_WaitForYourePlayerText(u8);
static void Task_NewGameCarterSpeech_StartNamingScreen(u8);
static void CB2_NewGameCarterSpeech_ReturnFromNamingScreen(void);
static void NewGameCarterSpeech_SetDefaultPlayerName(u8);
static void Task_NewGameCarterSpeech_CreateNameYesNo(u8);
static void Task_NewGameCarterSpeech_ProcessNameYesNoMenu(u8);
void CreateYesNoMenuParameterized(u8, u8, u16, u16, u8, u8);
static void Task_NewGameCarterSpeech_SlidePlatformAway2(u8);
static void Task_NewGameCarterSpeech_ReshowCarterLotad(u8);
static void Task_NewGameCarterSpeech_WaitForSpriteFadeInAndTextPrinter(u8);
static void Task_NewGameCarterSpeech_AreYouReady(u8);
static void Task_NewGameCarterSpeech_ShrinkPlayer(u8);
static void SpriteCB_MovePlayerDownWhileShrinking(struct Sprite *);
static void Task_NewGameCarterSpeech_WaitForPlayerShrink(u8);
static void Task_NewGameCarterSpeech_FadePlayerToWhite(u8);
static void Task_NewGameCarterSpeech_Cleanup(u8);
static void SpriteCB_Null(struct Sprite *);
static void Task_NewGameCarterSpeech_ReturnFromNamingScreenShowTextbox(u8);
static void Task_NewGameCarterSpeech_SoItsPlayerName(u8);
static void MainMenu_FormatSavegamePlayer(void);
static void MainMenu_FormatSavegamePokedex(void);
static void MainMenu_FormatSavegameTime(void);
static void MainMenu_FormatSavegameBadges(void);
static void NewGameCarterSpeech_CreateDialogueWindowBorder(u8, u8, u8, u8, u8, u8);

// .rodata

static const u16 sCarterSpeechBgPals[][16] = {
    INCGFX_U16("graphics/carter_speech/bg0.pal", ".gbapal"),
    INCGFX_U16("graphics/carter_speech/bg1.pal", ".gbapal")
};

static const u32 sCarterSpeechShadowGfx[] = INCGFX_U32("graphics/carter_speech/shadow.png", ".4bpp.lz");
static const u32 sCarterSpeechBgMap[] = INCGFX_U32("graphics/carter_speech/map.bin", ".lz");
static const u16 sCarterSpeechBgGradientPal[] = INCGFX_U16("graphics/carter_speech/bg2.pal", ".gbapal");
...
static const struct WindowTemplate sNewGameCarterSpeechTextWindows[] =
{
    {
        .bg = 0,
        .tilemapLeft = 2,
...
static const struct BgTemplate sCarterBgTemplate = {
    .bg = 0,
    .charBaseIndex = 3,
    .mapBaseIndex = 30,
    .screenSize = 0,
...
static const struct MenuAction sMenuActions_Gender[] = {
    {gText_CarterBoy, {NULL}},
    {gText_CarterGirl, {NULL}}
};
...
#define CARTER_DLG_BASE_TILE_NUM 0xFC
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                gTasks[taskId].func = Task_NewGameCarterSpeech_Init;
                break;
...
#define tCarterSpriteId data[8]
#define tLotadSpriteId data[9]
#define tBrendanSpriteId data[10]
#define tMaySpriteId data[11]

static void Task_NewGameCarterSpeech_Init(u8 taskId)
{
    SetGpuReg(REG_OFFSET_DISPCNT, 0);
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
    InitBgFromTemplate(&sCarterBgTemplate);
    SetGpuReg(REG_OFFSET_WIN0H, 0);
    SetGpuReg(REG_OFFSET_WIN0V, 0);
    SetGpuReg(REG_OFFSET_WININ, 0);
    SetGpuReg(REG_OFFSET_WINOUT, 0);
    ...
    LZ77UnCompVram(sCarterSpeechShadowGfx, (void *)VRAM);
    LZ77UnCompVram(sCarterSpeechBgMap, (void *)(BG_SCREEN_ADDR(7)));
    LoadPalette(sCarterSpeechBgPals, BG_PLTT_ID(0), 2 * PLTT_SIZE_4BPP);
    LoadPalette(&sCarterSpeechBgGradientPal[8], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ScanlineEffect_Stop();
    ResetSpriteData();
    FreeAllSpritePalettes();
    ResetAllPicSprites();
    AddCarterSpeechObjects(taskId);
    BeginNormalPaletteFade(PALETTES_ALL, 0, 16, 0, RGB_BLACK);
    gTasks[taskId].tBG1HOFS = 0;
    gTasks[taskId].func = Task_NewGameCarterSpeech_WaitToShowCarter;
...
static void Task_NewGameCarterSpeech_WaitToShowCarter(u8 taskId)
{
    u8 spriteId;
    ...
    {
        spriteId = gTasks[taskId].tCarterSpriteId;
        gSprites[spriteId].x = 136;
        gSprites[spriteId].y = 60;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameCarterSpeech_StartFadeInTarget1OutTarget2(taskId, 10);
        NewGameCarterSpeech_StartFadePlatformOut(taskId, 20);
        gTasks[taskId].tTimer = 80;
        gTasks[taskId].func = Task_NewGameCarterSpeech_WaitForSpriteFadeInWelcome;
    }
}

static void Task_NewGameCarterSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tCarterSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
        ...
        {
            InitWindows(sNewGameCarterSpeechTextWindows);
            LoadMainMenuWindowFrameTiles(0, 0xF3);
            LoadMessageBoxGfx(0, CARTER_DLG_BASE_TILE_NUM, BG_PLTT_ID(15));
            NewGameCarterSpeech_ShowDialogueWindow(0, 1);
            PutWindowTilemap(0);
            CopyWindowToVram(0, COPYWIN_GFX);
            NewGameCarterSpeech_ClearWindow(0);
            StringExpandPlaceholders(gStringVar4, gText_Carter_Welcome);
            AddTextPrinterForMessage(TRUE);
            gTasks[taskId].func = Task_NewGameCarterSpeech_ThisIsAPokemon;
        }
    }
}

static void Task_NewGameCarterSpeech_ThisIsAPokemon(u8 taskId)
{
    if (!gPaletteFade.active && !RunTextPrintersAndIsPrinter0Active())
    {
        gTasks[taskId].func = Task_NewGameCarterSpeech_MainSpeech;
        StringExpandPlaceholders(gStringVar4, gText_ThisIsAPokemon);
        AddTextPrinterWithCallbackForMessage(TRUE, NewGameCarterSpeech_WaitForThisIsPokemonText);
        sCarterSpeechMainTaskId = taskId;
    }
}

static void Task_NewGameCarterSpeech_MainSpeech(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        StringExpandPlaceholders(gStringVar4, gText_Carter_MainSpeech);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameCarterSpeech_AndYouAre;
    }
}

#define tState data[0]

static void Task_NewGameCarterSpeechSub_InitPokeBall(u8 taskId)
{
    u8 spriteId = gTasks[sCarterSpeechMainTaskId].tLotadSpriteId;
    ...
    gTasks[taskId].func = Task_NewGameCarterSpeechSub_WaitForLotad;
    gTasks[sCarterSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameCarterSpeechSub_WaitForLotad(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sCarterSpeechMainTaskId].tLotadSpriteId];
    ...
    switch (tState)
    {
        case 0:
            ...
            break;
        case 1:
            if (gTasks[sCarterSpeechMainTaskId].tTimer >= 96)
            {
                DestroyTask(taskId);
                if (gTasks[sCarterSpeechMainTaskId].tTimer < 0x4000)
                    gTasks[sCarterSpeechMainTaskId].tTimer++;
            }
            return;
    }
    tState++;
...
static void Task_NewGameCarterSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        sStartedPokeBallTask = FALSE;
        sBralenNameChosen = FALSE;
        StringExpandPlaceholders(gStringVar4, gText_Carter_AndYouAre);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameCarterSpeech_WaitThenShowBralenMenu;
    }
}

static void Task_NewGameCarterSpeech_WaitThenShowBralenMenu(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameCarterSpeech_ProcessBralenMenu;
    }
}

static void Task_NewGameCarterSpeech_ProcessBralenMenu(u8 taskId)
{
    u8 i;
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
        case 0: // Yes — set name and gender, skip straight to YourePlayer text
            ...
            NewGameCarterSpeech_ClearWindow(0);
            StringExpandPlaceholders(gStringVar4, gText_Carter_YourePlayer);
            AddTextPrinterForMessage(TRUE);
            gTasks[taskId].func = Task_NewGameCarterSpeech_WaitForYourePlayerText;
            break;
        case MENU_B_PRESSED:
        case 1: // No — loop with a persuasive message
            PlaySE(SE_SELECT);
            NewGameCarterSpeech_ClearWindow(0);
            StringExpandPlaceholders(gStringVar4, gText_Carter_NotBralen);
            AddTextPrinterForMessage(TRUE);
            gTasks[taskId].func = Task_NewGameCarterSpeech_WaitThenShowBralenMenu;
            break;
    }
...
static void Task_NewGameCarterSpeech_StartCarterLotadPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tCarterSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tLotadSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameCarterSpeech_StartFadeOutTarget1InTarget2(taskId, 2);
        NewGameCarterSpeech_StartFadePlatformIn(taskId, 1);
        gTasks[taskId].tTimer = 64;
        gTasks[taskId].func = Task_NewGameCarterSpeech_SlidePlatformAway;
    }
}

static void Task_NewGameCarterSpeech_SlidePlatformAway(u8 taskId)
{
    ...
    else
    {
        ...
        gTasks[taskId].func = Task_NewGameCarterSpeech_StartPlayerFadeIn;
    }
}

static void Task_NewGameCarterSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tCarterSpriteId].invisible = TRUE;
        ...
        else
        {
            ...
            NewGameCarterSpeech_StartFadeInTarget1OutTarget2(taskId, 2);
            NewGameCarterSpeech_StartFadePlatformOut(taskId, 1);
            gTasks[taskId].func = Task_NewGameCarterSpeech_WaitForPlayerFadeIn;
        }
    }
}

static void Task_NewGameCarterSpeech_WaitForPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gTasks[taskId].func = Task_NewGameCarterSpeech_BoyOrGirl;
    }
}

static void Task_NewGameCarterSpeech_WaitForYourePlayerText(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tCarterSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        ...
        NewGameCarterSpeech_StartFadeOutTarget1InTarget2(taskId, 2);
        gTasks[taskId].tTimer = 64;
        gTasks[taskId].func = Task_NewGameCarterSpeech_AreYouReady;
    }
}

static void Task_NewGameCarterSpeech_BoyOrGirl(u8 taskId)
{
    NewGameCarterSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Carter_BoyOrGirl);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameCarterSpeech_WaitToShowGenderMenu;
}

static void Task_NewGameCarterSpeech_WaitToShowGenderMenu(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        NewGameCarterSpeech_ShowGenderMenu();
        gTasks[taskId].func = Task_NewGameCarterSpeech_ChooseGender;
    }
}

static void Task_NewGameCarterSpeech_ChooseGender(u8 taskId)
{
    int gender = NewGameCarterSpeech_ProcessGenderMenuInput();
    int gender2;
    ...
    switch (gender)
    {
        case MALE:
            ...
            NewGameCarterSpeech_ClearGenderWindow(1, 1);
            gTasks[taskId].func = Task_NewGameCarterSpeech_WhatsYourName;
            break;
        case FEMALE:
            PlaySE(SE_SELECT);
            gSaveBlock2Ptr->playerGender = gender;
    ...
    if (gender2 != gTasks[taskId].tPlayerGender)
    {
        ...
        NewGameCarterSpeech_StartFadeOutTarget1InTarget2(taskId, 0);
        gTasks[taskId].func = Task_NewGameCarterSpeech_SlideOutOldGenderSprite;
    }
}

static void Task_NewGameCarterSpeech_SlideOutOldGenderSprite(u8 taskId)
{
    ...
    else
    {
        ...
        gTasks[taskId].tPlayerSpriteId = spriteId;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        NewGameCarterSpeech_StartFadeInTarget1OutTarget2(taskId, 0);
        gTasks[taskId].func = Task_NewGameCarterSpeech_SlideInNewGenderSprite;
    }
}

static void Task_NewGameCarterSpeech_SlideInNewGenderSprite(u8 taskId)
{
    u8 spriteId = gTasks[taskId].tPlayerSpriteId;
...
static void Task_NewGameCarterSpeech_WhatsYourName(u8 taskId)
{
    NewGameCarterSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Carter_WhatsYourName);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameCarterSpeech_WaitForWhatsYourNameToPrint;
}

static void Task_NewGameCarterSpeech_WaitForWhatsYourNameToPrint(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
        gTasks[taskId].func = Task_NewGameCarterSpeech_WaitPressBeforeNameChoice;
}

static void Task_NewGameCarterSpeech_WaitPressBeforeNameChoice(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        gTasks[taskId].func = Task_NewGameCarterSpeech_StartNamingScreen;
    }
}

static void Task_NewGameCarterSpeech_StartNamingScreen(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tLotadSpriteId);
        NewGameCarterSpeech_SetDefaultPlayerName(Random() % NUM_PRESET_NAMES);
        DestroyTask(taskId);
        DoNamingScreen(NAMING_SCREEN_PLAYER, gSaveBlock2Ptr->playerName, gSaveBlock2Ptr->playerGender, 0, 0, CB2_NewGameCarterSpeech_ReturnFromNamingScreen);
    }
}

static void Task_NewGameCarterSpeech_SoItsPlayerName(u8 taskId)
{
    NewGameCarterSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Carter_SoItsPlayer);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameCarterSpeech_CreateNameYesNo;
}

static void Task_NewGameCarterSpeech_CreateNameYesNo(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        CreateYesNoMenuParameterized(2, 1, 0xF3, 0xDF, 2, 15);
        gTasks[taskId].func = Task_NewGameCarterSpeech_ProcessNameYesNoMenu;
    }
}

static void Task_NewGameCarterSpeech_ProcessNameYesNoMenu(u8 taskId)
{
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
        case 0:
            ...
            NewGameCarterSpeech_StartFadeOutTarget1InTarget2(taskId, 2);
            NewGameCarterSpeech_StartFadePlatformIn(taskId, 1);
            gTasks[taskId].func = Task_NewGameCarterSpeech_SlidePlatformAway2;
            break;
        case MENU_B_PRESSED:
        case 1:
            PlaySE(SE_SELECT);
...
static void Task_NewGameCarterSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameCarterSpeech_ReshowCarterLotad;
    }
}

static void Task_NewGameCarterSpeech_ReshowCarterLotad(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        StringExpandPlaceholders(gStringVar4, gText_Carter_YourePlayer);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameCarterSpeech_WaitForSpriteFadeInAndTextPrinter;
    }
}

static void Task_NewGameCarterSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        gSprites[gTasks[taskId].tCarterSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
...
static void Task_NewGameCarterSpeech_AreYouReady(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        NewGameCarterSpeech_StartFadeInTarget1OutTarget2(taskId, 2);
        NewGameCarterSpeech_StartFadePlatformOut(taskId, 1);
        StringExpandPlaceholders(gStringVar4, gText_Carter_AreYouReady);
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameCarterSpeech_ShrinkPlayer;
    }
}

static void Task_NewGameCarterSpeech_ShrinkPlayer(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            ...
            FadeOutBGM(4);
            gTasks[taskId].func = Task_NewGameCarterSpeech_WaitForPlayerShrink;
        }
    }
}

static void Task_NewGameCarterSpeech_WaitForPlayerShrink(u8 taskId)
{
    u8 spriteId = gTasks[taskId].tPlayerSpriteId;

    if (gSprites[spriteId].affineAnimEnded)
        gTasks[taskId].func = Task_NewGameCarterSpeech_FadePlayerToWhite;
}

static void Task_NewGameCarterSpeech_FadePlayerToWhite(u8 taskId)
{
    u8 spriteId;
    ...
    if (!gPaletteFade.active)
    {
        ...
        gTasks[taskId].func = Task_NewGameCarterSpeech_Cleanup;
    }
}

static void Task_NewGameCarterSpeech_Cleanup(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        FreeAllWindowBuffers();
...
static void CB2_NewGameCarterSpeech_ReturnFromNamingScreen(void)
{
    ...
    ResetPaletteFade();
    LZ77UnCompVram(sCarterSpeechShadowGfx, (u8 *)VRAM);
    LZ77UnCompVram(sCarterSpeechBgMap, (u8 *)(BG_SCREEN_ADDR(7)));
    LoadPalette(sCarterSpeechBgPals, BG_PLTT_ID(0), 2 * PLTT_SIZE_4BPP);
    LoadPalette(&sCarterSpeechBgGradientPal[1], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    ResetTasks();
    taskId = CreateTask(Task_NewGameCarterSpeech_ReturnFromNamingScreenShowTextbox, 0);
...
static u8 NewGameCarterSpeech_CreateLotadSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_LOTAD, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}

static void AddCarterSpeechObjects(u8 taskId)
{
    u8 carterSpriteId;
    u8 lotadSpriteId;
    u8 brendanSpriteId;
    ...
    carterSpriteId = AddNewGameCarterObject(0x88, 0x3C, 1);
    gSprites[carterSpriteId].callback = SpriteCB_Null;
    gSprites[carterSpriteId].oam.priority = 0;
    gSprites[carterSpriteId].invisible = TRUE;
    gTasks[taskId].tCarterSpriteId = carterSpriteId;
    lotadSpriteId = NewGameCarterSpeech_CreateLotadSprite(100, 0x4B);
...
static void Task_NewGameCarterSpeech_FadeOutTarget1InTarget2(u8 taskId)
{
    int alphaCoeff2;
...
static void NewGameCarterSpeech_StartFadeOutTarget1InTarget2(u8 taskId, u8 delay)
{
    u8 taskId2;
    ...
    taskId2 = CreateTask(Task_NewGameCarterSpeech_FadeOutTarget1InTarget2, 0);
...
static void Task_NewGameCarterSpeech_FadeInTarget1OutTarget2(u8 taskId)
{
    int alphaCoeff2;
...
static void NewGameCarterSpeech_StartFadeInTarget1OutTarget2(u8 taskId, u8 delay)
{
    u8 taskId2;
    ...
    SetGpuReg(REG_OFFSET_BLDY, 0);
    gTasks[taskId].tIsDoneFadingSprites = 0;
    taskId2 = CreateTask(Task_NewGameCarterSpeech_FadeInTarget1OutTarget2, 0);
    gTasks[taskId2].tMainTask = taskId;
...
static void Task_NewGameCarterSpeech_FadePlatformIn(u8 taskId)
{
    ...
    else
    {
        ...
        LoadPalette(&sCarterSpeechBgGradientPal[gTasks[taskId].tPalIndex], BG_PLTT_ID(0) + 1, PLTT_SIZEOF(8));
    }
}

static void NewGameCarterSpeech_StartFadePlatformIn(u8 taskId, u8 delay)
{
    u8 taskId2;

    taskId2 = CreateTask(Task_NewGameCarterSpeech_FadePlatformIn, 0);
    gTasks[taskId2].tMainTask = taskId;
...
static void Task_NewGameCarterSpeech_FadePlatformOut(u8 taskId)
{
    if (gTasks[taskId].tDelayBefore)
    {
        gTasks[taskId].tDelayBefore--;
...
static void NewGameCarterSpeech_StartFadePlatformOut(u8 taskId, u8 delay)
{
    u8 taskId2;

    taskId2 = CreateTask(Task_NewGameCarterSpeech_FadePlatformOut, 0);
    gTasks[taskId2].tMainTask = taskId;
...
static void NewGameCarterSpeech_ShowGenderMenu(void)
{
    DrawMainMenuWindowBorder(&sNewGameCarterSpeechTextWindows[1], 0xF3);
...
static void NewGameCarterSpeech_SetDefaultPlayerName(u8 nameId)
{
    const u8 *name;
    u8 i;
...
static void NewGameCarterSpeech_ClearGenderWindowTilemap(u8 bg, u8 x, u8 y, u8 width, u8 height, u8 unused)
{
    FillBgTilemapBufferRect(bg, 0, x + 255, y + 255, width + 2, height + 2, 2);
}

static void NewGameCarterSpeech_ClearGenderWindow(u8 windowId, bool8 copyToVram)
{
    CallWindowFunction(windowId, NewGameCarterSpeech_ClearGenderWindowTilemap);
...
static void NewGameCarterSpeech_ClearWindow(u8 windowId)
{
...
static void NewGameCarterSpeech_WaitForThisIsPokemonText(struct TextPrinterTemplate *printer, u16 renderCmd)
{
    // Wait for Carter's "This is a Pokémon" text to reach the pause
    ...
    if (*(printer->currentChar - 2) == EXT_CTRL_CODE_PAUSE && !sStartedPokeBallTask)
    {
        ...
        CreateTask(Task_NewGameCarterSpeechSub_InitPokeBall, 0);
    }
}
...
static void NewGameCarterSpeech_ShowDialogueWindow(u8 windowId, u8 copyToVram)
{
    CallWindowFunction(windowId, NewGameCarterSpeech_CreateDialogueWindowBorder);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
...
static void NewGameCarterSpeech_CreateDialogueWindowBorder(u8 bg, u8 x, u8 y, u8 width, u8 height, u8 palNum)
{
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  1, x-2,       y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  3, x-1,       y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  4, x,         y-1, width,   1, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  5, x+width-1, y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  6, x+width,   y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  7, x-2,       y,   1,       5, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM +  9, x-1,       y,   width+1, 5, palNum);
    FillBgTilemapBufferRect(bg, CARTER_DLG_BASE_TILE_NUM + 10, x+width,   y,   1,       5, palNum);

    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(CARTER_DLG_BASE_TILE_NUM + 1), x-2,       y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(CARTER_DLG_BASE_TILE_NUM + 3), x-1,       y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(CARTER_DLG_BASE_TILE_NUM + 4), x,         y+height, width-1, 1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(CARTER_DLG_BASE_TILE_NUM + 5), x+width-1, y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(CARTER_DLG_BASE_TILE_NUM + 6), x+width,   y+height, 1,       1, palNum);
}

static void Task_NewGameCarterSpeech_ReturnFromNamingScreenShowTextbox(u8 taskId)
{
    if (gTasks[taskId].tTimer-- <= 0)
    {
        NewGameCarterSpeech_ShowDialogueWindow(0, 1);
        gTasks[taskId].func = Task_NewGameCarterSpeech_SoItsPlayerName;
    }
}
